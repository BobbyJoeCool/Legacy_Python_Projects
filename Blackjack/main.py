...
while playing == True:
    ...
    while playerTurn:
        if player.hand.blackjack():  # Check for Player Blackjack
            ...
            if player.chips >= player.hand.bet * 2:
                if (
                    player.hand.cards[0].rank == player.hand.cards[1].rank
                    and len(player.hand.cards) == 2
                ):
                    split = input(
                        "WOuld you like to Split you hand and place an equal bet on the second hand? (y/n): "
                    )
                ...
                if len(player.hand.cards) == 2 and split != "y":
                    double = input(
                        "Would you like to double down, double your bet, hit one time, and be done? (y/n): "
                    )
    ...
    if player.chips < 5:
        print(
            "Sorry, you do not have enough chips to continue playing.  Thanks for playing!"
        )
        break
