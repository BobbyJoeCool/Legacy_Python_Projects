...
Blackjack pays 1.5x the bet (Rounded Down).""",
        ...
        self.nameLabel = ttk.Label(
            self.welcomeFrame, text="What is your Name:", justify="right"
        )
        self.nameEntry = ttk.Entry(self.welcomeFrame, textvariable=playerName)
        self.decksLabel = ttk.Label(
            self.welcomeFrame,
            text="How many Decks do you want to play with?",
            justify="right",
        )
        self.decksEntry = ttk.Entry(self.welcomeFrame, textvariable=decks)
        self.startGame = ttk.Button(
            self.welcomeFrame,
            command=lambda: self.beginGame(playerName.get(), decks.get()),
            text="Start Game",
        )
        self.quit = ttk.Button(
            self.welcomeFrame, command=self.root.destroy, text="Quit"
        )
