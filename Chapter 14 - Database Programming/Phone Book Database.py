...
phone_database = [
    ("Breutzmann, Robert", "319-939-3687"),
...
def add_db():
    if is_valid_phone(phone.get()) == False:
        messagebox.showerror(
            "Invalid Input", "Phone number must be in the format ###-###-####."
        )
...
def edit_db():
    ...
    if not noPhoneChanges and noNameChanges:
        cur.execute(
            """UPDATE Entries SET Phone = ? WHERE Name = ?""", (newphone, curname)
        )
        messagebox.showinfo(
            "Success", f"Entry updated:\nName: {curname}\nUpdated Phone: {newphone}"
        )
    elif not noNameChanges and noPhoneChanges:
        cur.execute(
            """UPDATE Entries SET Name = ? WHERE Name = ?""", (newname, curname)
        )
        messagebox.showinfo(
        ...
        cur.execute(
            """UPDATE Entries SET Name = ?, Phone = ? WHERE Name = ?""",
            (newname, newphone, curname),
        )
        messagebox.showinfo(
            "Success",
            f"Entry updated:\nUpdated Name: {newname}\nUpdated Phone: {newphone}",
...
def delete_db():
    ...
    response = messagebox.askokcancel(
        "Are you sure?",
        f"You are about to delete this entry! \nName: {curname}\nPhone: {curphone}\nAre you sure?",
    )
    if response == True:
...
def add_entry():
    ...
    submitButton = tk.Button(
        win, text="Submit", command=lambda: (add_db(), win.destroy())
    )
...
def lookup_entry():
    ...
    phone_label = tk.Label(
        win, text=name_to_phone.get(selected_name.get(), ""), width=30
    )
...
def edit_entry():
    ...
    changeNameLabel = tk.Label(
        win, text="Change Name to (Leave blank to keep)", width=30
    )
    changeNameLabel.grid(column=0, row=1, sticky="s", pady=10)
    changePhoneLabel = tk.Label(
        win, text="Chane Phone to (leave blank to keep)", width=30
    )
    ...
    # Create a button
    submitButton = tk.Button(
        win, text="Submit", command=lambda: (edit_db(), win.destroy())
    )
...
def delete_entry():
    ...
    deleteButton = tk.Button(
        win, text="Delete Entry", command=lambda: (delete_db(), win.destroy())
    )
