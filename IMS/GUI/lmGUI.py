...
class LocationManagerGUI:
        ...
        def findAisle(event=None):
            if val.StorageCode(self.storageCode.get()):
                if val.Size(self.size.get()):
                    ...
                    displayBox = ttk.Treeview(
                        elaFrame, columns=columns, show="headings"
                    )
    ...
    def buildELZ(self):
        ...
        centerWrapper.pack(expand=True, fill="both")
        self.elzFrame = ttk.Frame(
            centerWrapper, padding=10, relief="ridge", borderwidth=2
        )
    ...
    def findAisle(self, event=None):
        ...
        emptyLocations = ttk.Treeview(
            self.elzFrame, columns=emptyLocationsColumns, show="headings"
        )
        ...
        oddSideColumnsSetup = ttk.Treeview(
            self.elzFrame, columns=oddSideColumns, show="headings"
        )
        ...
        evenSideColumnsSetup = ttk.Treeview(
            self.elzFrame, columns=evenSideColumns, show="headings"
        )
    ...
    def buildWLI(self):
        ...
        displayMessageDisplay = ttk.Label(
            wliFrame, textvariable=displayMessage, font=self.bold_font
        )
        ...
        def findLocation(self, aisle, bin, level, event=None):
            if val.LocationExists(aisle, bin, level):
                ...
                if palletInfo:
                    name = getData.DPCIgetName(
                        palletInfo["Dept"], palletInfo["Class"], palletInfo["Item"]
                    )
        ...
        aisleEntry.bind(
            "<Return>",
            lambda event: findLocation(
                self, aisle.get(), bin.get(), level.get(), event
            ),
        )
        binEntry.bind(
            "<Return>",
