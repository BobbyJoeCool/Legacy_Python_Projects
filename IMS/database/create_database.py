...
def dropSelectedTables():
    ...
    if not selected:
        messagebox.showwarning(
            "No Selection", "Please select at least one table to delete."
        )
        return

    confirm = messagebox.askyesno(
        "Confirm", f"Delete the following tables?\n\n{', '.join(selected)}"
    )
    if confirm:
...
def populate_storageCodes():
    ...
    try:
        ...
        messagebox.showerror(
            "Database Error", f"Failed to generate storage codes:\n{e}"
        )
...
def populate_items():
    try:
        ...
        if count == 0:
            messagebox.showerror(
                "Missing Data", "Please populate Storage Codes before adding items."
            )
            return
...
def populate_pallets(fill_percent=0.9, created_by=1):
    ...
    db.commit()
    messagebox.showinfo(
        "Done", f"✅ Populated {filled} pallets into locations (~{fill_percent*100}%)."
    )
...
def generateRandomData(selected_checkboxes):
    ...
    if not selected:
        messagebox.showwarning(
            "No Selection", "Please select at least one table to generate data for."
        )
        return

    confirm = messagebox.askyesno(
        "Confirm", f"Generate random data for: {', '.join(selected)}?"
    )
    if not confirm:
        return
...
def export_database_to_excel():
    try:
        ...
        messagebox.showinfo(
            "Success", f"Database exported successfully to:\n{file_path}"
        )
...
title_label = ttk.Label(
    main_frame, text="Warehouse Database Manager", font=("Helvetica", 16, "bold")
)
title_label.pack(pady=(0, 15))

# Buttons
ttk.Button(main_frame, text="Create Tables", command=createTables).pack(
    pady=5, fill="x"
)
...
ttk.Button(main_frame, text="Delete Selected Tables", command=dropSelectedTables).pack(
    pady=10, fill="x"
)
...
ttk.Button(
    main_frame,
    text="Generate Data for Selected Tables",
    command=lambda: generateRandomData(rand_checkboxes),
).pack(pady=5, fill="x")

ttk.Button(
    main_frame, text="Export Database to Excel", command=export_database_to_excel
).pack(pady=5, fill="x")
