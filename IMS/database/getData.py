...
def LocGetPallet(aisle, bin, level):
    ...
    db, cur = getConn()
    cur.execute(
        "SELECT * FROM pallet WHERE Aisle = ? AND Bin = ? AND Level = ?",
        (aisle, bin, level),
    )
    row = cur.fetchone()
    db.close()
...
def GetItemInfo(dept, cls, item):
    ...
    db, cur = getConn()
    cur.execute(
        "SELECT * FROM items WHERE Dept = ? AND Class = ? AND Item = ?",
        (dept, cls, item),
    )
    row = cur.fetchone()
    db.close()
...
def DPCIgetName(dept, cls, item):
    ...
    db, cur = getConn()
    cur.execute(
        "SELECT Name FROM items WHERE Dept = ? AND Class = ? AND Item = ?",
        (dept, cls, item),
    )
    row = cur.fetchone()
    db.close()
...
def StorageRestricted(storageCode):
    ...
    db, cur = getConn()
    cur.execute(
        "SELECT Restricted FROM storageCodes WHERE StorageCode = ?", (storageCode,)
    )
    row = cur.fetchone()
    db.close()
    return row[0] if row else None
...
def GetLocationInfo(aisle, bin, level):
    ...
    db, cur = getConn()
    cur.execute(
        "SELECT * FROM locations WHERE Aisle = ? AND Bin = ? AND Level = ?",
        (aisle, bin, level),
    )
    row = cur.fetchone()
    db.close()
...
def LocGetStatus(aisle, bin, level):
    ...
    db, cur = getConn()
    cur.execute(
        "SELECT Status FROM locations WHERE Aisle = ? AND Bin = ? AND Level = ?",
        (aisle, bin, level),
    )
    row = cur.fetchone()
    db.close()
...
def LocGetStorage(aisle, bin, level):
    ...
    db, cur = getConn()
    cur.execute(
        "SELECT StorageCode FROM locations WHERE Aisle = ? AND Bin = ? AND Level = ?",
        (aisle, bin, level),
    )
    row = cur.fetchone()
    db.close()
