...
class GUI:
    def __init__(self):
        ...
        self.label_frame = tk.Frame(
            self.main_window, bg="lightblue"
        )  # Creates a Light Blue Frame
        self.label_frame.pack()
        self.check_frame = tk.Frame(self.main_window, bg="blue")  # Creates a Blue Frame
        self.check_frame.pack()
        self.combo_frame = tk.Frame(
            self.main_window, bg="green"
        )  # Creates a Green Frame
        ...
        label = tk.Label(
            ...
            bg="lightblue",
            fg="black",
        )  # Makes a label showing Hello
        label.config(text="This is a Test")  # Changes the text to Goodbye
        label.config(
            anchor="w"
        )  # Aligns test to the "west (left)" of the screen ("center" for center)
        ...
        checkbox = tk.Checkbutton(
            self.check_frame, text="Check me! (Checked)", bg="blue", variable=cked
        )
        checkbox.select()  # Selects the checkbox, Deselect deselects the box.
        checkbox.config(
            command=show_state
        )  # This calls the function "show_state" when the checkbox is ticked.
        ...
        combobox.set("Alpha")
        combobox.bind(
            "<<ComboboxSelected>>", dd_selection_changed
        )  # This calls the function when the selection changes.
